...
const ComplainItem = (props)=> {
    ...
    const complainRemoveItem = (email) => {
    ...
    const complainAllowHandler = async (carData) => {
     ...
     complainRemoveItem(carData.email)
     return res
  
    }
  
    const complainNotAllowHandler =  (email) => {
      complainRemoveItem(email) 
    }
    ...
    const getAvatarUrl = (car) => {
      ...
      return (
          <>
            <TableRow>
              <TableCell>
                <Avatar alt={props.complain.name} src={getAvatarUrl(props.complain)} />
              </TableCell>
              <TableCell>{props.complain.vin}</TableCell>
              <TableCell>{props.complain.email}</TableCell>
              
              <TableCell>
                <button onClick={() => complainAllowHandler(props.complain)} className="btn btn-outline-danger my-2 mx-2" style={{'borderRadius':'50px',}}>accept</button>
              </TableCell>

              <TableCell>            
                <button onClick={() => complainNotAllowHandler(props.complain.email)} className="btn btn-outline-danger my-2 mx-2" style={{'borderRadius':'50px',}}>reject</button>
              </TableCell>
